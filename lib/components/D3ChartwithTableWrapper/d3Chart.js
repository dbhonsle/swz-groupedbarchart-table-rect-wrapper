...
var d3Chart = function (_EventEmitter) {
    _inherits(d3Chart, _EventEmitter);

    function d3Chart(props) {
        _classCallCheck(this, d3Chart);

        var _this = _possibleConstructorReturn(this, (d3Chart.__proto__ || Object.getPrototypeOf(d3Chart)).call(this));
        ...
        _this.margin = { top: 20, bottom: 20, left: 150, right: 9 };
        _this._formatAbbreviation = _this._formatAbbreviation.bind(_this);
        return _this;
    }
    ...
    _createClass(d3Chart, [{
        ...
        value: function create(el, props, state) {
            ...
            this.svg = this.container.append('svg').attr('class', 'd3-chart').attr('width', this.width).attr('height', this.height);

            // chart plot area
            this.chart = this.svg.append('g').attr('class', 'bars').attr('transform', "translate(" + this.margin.left + "," + this.margin.top + ")");
        ...
        value: function _draw(state) {
            ...
            var bargroups = this.chart.selectAll('.bargroup').data(state.data);
        ...
        value: function update(el, state) {
            ...
            // Remove exiting bars
            bars.exit().transition().duration(300).attr("y", this.yScale(0)).attr("height", this.height - this.yScale(0)).style('fill-transparancy', 1e-6).remove();
        ...
        value: function _formatAbbreviation(x) {
            var v = Math.abs(x);
            return (v >= .9995e7 ? this._formatCrore : v >= .9995e5 ? this._formatLakh : v >= .9995e3 ? this._formatThousand : this._formatHundred)(x);
        }
    }, {
        key: '_formatCrore',
