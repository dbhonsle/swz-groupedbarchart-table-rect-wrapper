...
class d3Chart extends EventEmitter  {
    ...
    create(el, props, state) {
        ...
        let maxY = [0, d3.max(state.data, function(d) { 
            return d3.max(d.values, function(e) { return +e.value}) } )];

        let tickValues = this._getTicks(maxY[1]);
        let yDomain = [0, tickValues[tickValues.length - 1]];

        let keys = state.data[0].values.map(function(obj) {return obj.name});
        ...
        this.x1Scale = d3.scaleBand()
            .padding(0.05).domain(x1Domain).rangeRound([0, this.xScale.bandwidth()]);
        ...
        .attr('transform', "translate("+ this.margin.left + "," + this.margin.top +")")
        .call(d3.axisLeft(this.yScale).tickValues(tickValues).tickSize(-(this.width - padding) ).tickFormat(this._formatAbbreviation ) );        
        this._draw(state);
    }


    _draw(state) {
        let bargroups = this.chart.selectAll('.bargroup')
            .data(state.data);
        ...
        bargroups
            .enter().append("g")
            .attr('class', 'bargroup')
                .attr("transform", d => { return "translate(" + this.xScale(d.index) + ",0)"; })
                .attr("id", d => { return d.id })
            .selectAll("rect")
            .data(function(d) {return d.values})
            .enter().append("rect")
                .attr('class', 'bar')
                .attr("x", d => { return this.x1Scale(d.name); })
                ...
                .attr("fill", d => { return this.colors(d.name); });

        let indexes = ['Products'];
        state.data.forEach( element => {indexes.push(element.index)});
        ...
        this.chartTable.selectAll("tbody tr") 
        .sort(function(a, b) {
    ...
    _formatAbbreviation(x) {
        let v = Math.abs(x);
        return (v >= .9995e7 ? this._formatCrore
            : v >= .9995e5 ? this._formatLakh
            : v >= .9995e3  ? this._formatThousand
            : this._formatHundred)(x);
    }

    _formatCrore(x) { return (x % 1e7) > 0 ?  d3.format(".1f")(x / 1e7) + "Cr" :  d3.format(".0f")(x / 1e7) + "Cr"; };
    _formatLakh(x) { return (x % 1e5) > 0 ?  d3.format(".1f")(x / 1e5) + "L" :  d3.format(".0f")(x / 1e5)   + "L"; };
    _formatThousand(x) { return (x % 1e3) > 0 ?  d3.format(".1f")(x / 1e3) + "k" : d3.format(".0f")(x / 1e3) + "k"; };
    _formatHundred(x) { return d3.format("d")(x); }
